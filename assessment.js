...
function longestString(strs) {
  ...
  for( let x= 0; x < strs.length;  x++){
    if (strs[x].length > largestStringLength){
      largestStringLength = strs[x].length
      largestString = strs[x];
    }
  }
...
function computeUserAverageAge(users) {
  ...
  let numberTotal = 0
    ...
    for(let i = 0; i < numArr.length; i++){
      numberTotal += numArr[i];
    }
    let avg = Math.ceil(numberTotal/ (numArr.length));
    return avg;
}
